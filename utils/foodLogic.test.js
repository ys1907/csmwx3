...
// ===== 场景/标签匹配（词表已全库归一，引擎按字面比较；旧词归一在 migrateFood，见 util.test.js） =====

test('matchesScene: scenes 数组是唯一匹配权威', () => {
  const food = { scene: '外卖', scenes: ['外卖', '堂食', '公司食堂'] }
  assert.strictEqual(matchesScene(food, '堂食'), true)
  assert.strictEqual(matchesScene(food, '公司食堂'), true)
...
test('filterFoods: 场景过滤按 scenes 命中（公司食堂/堂食均可达）', () => {
  const foods = [
    { name: '木须肉套餐', scene: '堂食', scenes: ['堂食', '公司食堂'], budget: '💰', time: '快', tags: [] },
...
test('availabilityLevel: 按统一词表 key 直查', () => {
  const food = { availability: { 外卖: '高', 堂食: '中', 自己做: '低', 公司食堂: '极低' } }
...
test('foodHasTag: 字面匹配 + 「辣」的 spicyLevel 语义兜底', () => {
  assert.strictEqual(foodHasTag({ tags: ['肉'] }, '肉'), true)
...
test('filterFoods: 避雷按统一词表精确拦截', () => {
  const foods = [
    { name: '酱大骨', scene: '堂食', budget: '💰💰', time: '慢', tags: ['肉'] },
    { name: '白灼菜心', scene: '自己做', budget: '💰', time: '快', tags: ['素', '清淡'] },
  ]
